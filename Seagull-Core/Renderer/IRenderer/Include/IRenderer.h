...
// NOTE: Setting this define will disable use of the KHR 
// dedicated allocation extension since it conflicts with the renderdoc capture layer
//#define USE_RENDER_DOC

// raytracing for vulkan
...
#include "../../../Core/Source/Core/CompilerConfig.h"
...
{
	SG_MARKER_TYPE_DEFAULT = 0x0,
	SG_MARKER_TYPE_IN = 0x1,
	SG_MARKER_TYPE_OUT = 0x2,
	SG_MARKER_TYPE_IN_OUT = 0x3,
} MarkerType;
...
typedef struct BinaryShaderDesc // all the shaders in the pipeline
{
	ShaderStage           stages;
...
// #mPayloadSize - Size of the payload(”––ß‘ÿ∫…) struct for passing data to and from the shaders.
...
typedef struct RendererDesc
{
	...
	bool                         eequestAllAvailableQueues;
#endif
#if defined(SG_GRAPHIC_API_D3D12)
...
/// use for DirectX 12 Multi-indirect Draw
...
//
API_INTERFACE void SG_CALLCONV init_renderer(const char* appName, const RendererDesc* pSettings, Renderer** pRenderer);
API_INTERFACE void SG_CALLCONV remove_renderer(Renderer* pRenderer);

API_INTERFACE void SG_CALLCONV add_fence(Renderer* pRenderer, Fence** pFence);
API_INTERFACE void SG_CALLCONV remove_fence(Renderer* pRenderer, Fence* pFence);

API_INTERFACE void SG_CALLCONV add_semaphore(Renderer* pRenderer, Semaphore** pSemaphore);
API_INTERFACE void SG_CALLCONV remove_semaphore(Renderer* pRenderer, Semaphore* pSemaphore);

API_INTERFACE void SG_CALLCONV add_queue(Renderer* pRenderer, QueueDesc* pQueueDesc, Queue** pQueue);
API_INTERFACE void SG_CALLCONV remove_queue(Renderer* pRenderer, Queue* pQueue);

API_INTERFACE void SG_CALLCONV add_swapChain(Renderer* pRenderer, const SwapChainDesc* pDesc, SwapChain** pSwapChain);
API_INTERFACE void SG_CALLCONV remove_swapChain(Renderer* pRenderer, SwapChain* pSwapChain);

// command pool functions
API_INTERFACE void SG_CALLCONV add_cmd_pool(Renderer* pRenderer, const CmdPoolDesc* pDesc, CmdPool** pCmdPool);
API_INTERFACE void SG_CALLCONV remove_cmd_pool(Renderer* pRenderer, CmdPool* pCmdPool);
API_INTERFACE void SG_CALLCONV add_cmd(Renderer* pRenderer, const CmdDesc* pDesc, Cmd** pCmd);
API_INTERFACE void SG_CALLCONV remove_cmd(Renderer* pRenderer, Cmd* pCmd);
API_INTERFACE void SG_CALLCONV add_cmd_n(Renderer* pRenderer, const CmdDesc* pDesc, uint32_t cmdCount, Cmd*** pCmds);
API_INTERFACE void SG_CALLCONV remove_cmd_n(Renderer* pRenderer, uint32_t cmdCount, Cmd** pCmds);

//
// All buffer, texture loading handled by resource system -> IResourceLoader.*
//

API_INTERFACE void SG_CALLCONV add_render_target(Renderer* pRenderer, const RenderTargetDesc* pDesc, RenderTarget** ppRenderTarget);
API_INTERFACE void SG_CALLCONV remove_render_target(Renderer* pRenderer, RenderTarget* pRenderTarget);
API_INTERFACE void SG_CALLCONV add_sampler(Renderer* pRenderer, const SamplerDesc* pDesc, Sampler** pSampler);
API_INTERFACE void SG_CALLCONV remove_sampler(Renderer* pRenderer, Sampler* pSampler);

// shader functions
#if defined(TARGET_IOS)
API_INTERFACE void SG_CALLCONV add_shader(Renderer* pRenderer, const ShaderDesc* pDesc, Shader** p_shader_program);
#endif
API_INTERFACE void SG_CALLCONV add_shader_binary(Renderer* pRenderer, const BinaryShaderDesc* pDesc, Shader** pShader);
API_INTERFACE void SG_CALLCONV remove_shader(Renderer* pRenderer, Shader* pShader);

API_INTERFACE void SG_CALLCONV add_root_signature(Renderer* pRenderer, const RootSignatureDesc* pDesc, RootSignature** pRootSignature);
API_INTERFACE void SG_CALLCONV remove_root_signature(Renderer* pRenderer, RootSignature* pRootSignature);

// pipeline functions
API_INTERFACE void SG_CALLCONV add_pipeline(Renderer* pRenderer, const PipelineDesc* pPipelineSettings, Pipeline** pPipeline);
API_INTERFACE void SG_CALLCONV remove_pipeline(Renderer* pRenderer, Pipeline* pPipeline);
API_INTERFACE void SG_CALLCONV add_pipeline_cache(Renderer* pRenderer, const PipelineCacheDesc* pDesc, PipelineCache** ppPipelineCache);
API_INTERFACE void SG_CALLCONV get_pipeline_cache_data(Renderer* pRenderer, PipelineCache* pPipelineCache, size_t* pSize, void* pData);
API_INTERFACE void SG_CALLCONV remove_pipeline_cache(Renderer* pRenderer, PipelineCache* pPipelineCache);

// Descriptor Set functions
API_INTERFACE void SG_CALLCONV add_descriptor_set(Renderer* pRenderer, const DescriptorSetDesc* pDesc, DescriptorSet** pDescriptorSet);
API_INTERFACE void SG_CALLCONV remove_descriptor_set(Renderer* pRenderer, DescriptorSet* pDescriptorSet);
API_INTERFACE void SG_CALLCONV update_descriptor_set(Renderer* pRenderer, uint32_t index, DescriptorSet* pDescriptorSet, uint32_t count, const DescriptorData* pParams);

// command buffer functions
API_INTERFACE void SG_CALLCONV reset_cmd_pool(Renderer* pRenderer, CmdPool* pCmdPool);
API_INTERFACE void SG_CALLCONV begin_cmd(Cmd* pCmd);
API_INTERFACE void SG_CALLCONV end_cmd(Cmd* pCmd);
API_INTERFACE void SG_CALLCONV cmd_bind_render_targets(Cmd* pCmd, uint32_t renderTargetCount, RenderTarget** pRenderTargets, RenderTarget* pDepthStencil, const LoadActionsDesc* loadActions, uint32_t* pColorArraySlices, uint32_t* pColorMipSlices, uint32_t depthArraySlice, uint32_t depthMipSlice);
API_INTERFACE void SG_CALLCONV cmd_set_viewport(Cmd* pCmd, float x, float y, float width, float height, float minDepth, float maxDepth);
API_INTERFACE void SG_CALLCONV cmd_set_scissor(Cmd* pCmd, uint32_t x, uint32_t y, uint32_t width, uint32_t height);
API_INTERFACE void SG_CALLCONV cmd_set_stencil_reference_value(Cmd* pCmd, uint32_t value);
API_INTERFACE void SG_CALLCONV cmd_bind_pipeline(Cmd* pCmd, Pipeline* p_pipeline);
API_INTERFACE void SG_CALLCONV cmd_bind_descriptor_set(Cmd* pCmd, uint32_t index, DescriptorSet* pDescriptorSet);
API_INTERFACE void SG_CALLCONV cmd_bind_push_constants(Cmd* pCmd, RootSignature* pRootSignature, const char* pName, const void* pConstants);
API_INTERFACE void SG_CALLCONV cmd_bind_push_constants_by_index(Cmd* pCmd, RootSignature* pRootSignature, uint32_t paramIndex, const void* pConstants);
API_INTERFACE void SG_CALLCONV cmd_bind_vertex_buffer(Cmd* pCmd, uint32_t bufferCount, Buffer** ppBuffers, const uint32_t* pStrides, const uint64_t* pOffsets);
API_INTERFACE void SG_CALLCONV cmd_bind_index_buffer(Cmd* pCmd, Buffer* pBuffer, uint32_t indexType, uint64_t offset);
API_INTERFACE void SG_CALLCONV cmd_draw(Cmd* pCmd, uint32_t vertex_count, uint32_t firstVertex);
API_INTERFACE void SG_CALLCONV cmd_draw_instanced(Cmd* pCmd, uint32_t vertexCount, uint32_t firstVertex, uint32_t instanceCount, uint32_t firstInstance);
API_INTERFACE void SG_CALLCONV cmd_draw_indexed(Cmd* pCmd, uint32_t indexCount, uint32_t firstIndex, uint32_t firstVertex);
API_INTERFACE void SG_CALLCONV cmd_draw_indexed_instanced(Cmd* pCmd, uint32_t indexCount, uint32_t firstIndex, uint32_t instanceCount, uint32_t firstVertex, uint32_t firstInstance);
API_INTERFACE void SG_CALLCONV cmd_dispatch(Cmd* pCmd, uint32_t groupCountX, uint32_t groupCountY, uint32_t groupCountZ);

// Transition Commands
API_INTERFACE void SG_CALLCONV cmd_resource_barrier(Cmd* pCmd, uint32_t bufferBarrierCount, BufferBarrier* pBufferBarriers, uint32_t textureBarrierCount, TextureBarrier* pTextureBarriers, uint32_t rtBarrierCount, RenderTargetBarrier* pRtBarriers);

// Virtual Textures
API_INTERFACE void SG_CALLCONV cmd_update_virtual_texture(Cmd* pCmd, Texture* pTexture);
//
// All buffer, texture update handled by resource system -> IResourceLoader.*
//

// queue/fence/swapchain functions
API_INTERFACE void SG_CALLCONV acquire_next_image(Renderer* pRenderer, SwapChain* pSwapChain, Semaphore* pSignalSemaphore, Fence* pFence, uint32_t* pImageIndex);
API_INTERFACE void SG_CALLCONV queue_submit(Queue* pQueue, const QueueSubmitDesc* pDesc);
API_INTERFACE PresentStatus SG_CALLCONV queue_present(Queue* pQueue, const QueuePresentDesc* pDesc);
API_INTERFACE void SG_CALLCONV wait_queue_idle(Queue* pQueue);
API_INTERFACE void SG_CALLCONV get_fence_status(Renderer* pRenderer, Fence* pFence, FenceStatus* pFenceStatus);
API_INTERFACE void SG_CALLCONV wait_for_fences(Renderer* pRenderer, uint32_t fenceCount, Fence** ppFences);
API_INTERFACE void SG_CALLCONV toggle_VSync(Renderer* pRenderer, SwapChain** ppSwapchain);
...
API_INTERFACE TinyImageFormat SG_CALLCONV get_recommended_swapchain_format(bool hintHDR);

//indirect Draw functions
API_INTERFACE void SG_CALLCONV add_indirect_command_signature(Renderer* pRenderer, const CommandSignatureDesc* pDesc, CommandSignature** ppCommandSignature);
API_INTERFACE void SG_CALLCONV remove_indirect_command_signature(Renderer* pRenderer, CommandSignature* pCommandSignature);
API_INTERFACE void SG_CALLCONV cmd_execute_indirect(Cmd* pCmd, CommandSignature* pCommandSignature, uint32_t maxCommandCount, Buffer* pIndirectBuffer, uint64_t bufferOffset, Buffer* pCounterBuffer, uint64_t counterBufferOffset);

/************************************************************************/
// GPU Query Interface
/************************************************************************/
API_INTERFACE void SG_CALLCONV get_timestamp_frequency(Queue* pQueue, double* pFrequency);
API_INTERFACE void SG_CALLCONV add_query_pool(Renderer* pRenderer, const QueryPoolDesc* pDesc, QueryPool** ppQueryPool);
API_INTERFACE void SG_CALLCONV remove_query_pool(Renderer* pRenderer, QueryPool* pQueryPool);
API_INTERFACE void SG_CALLCONV cmd_reset_query_pool(Cmd* pCmd, QueryPool* pQueryPool, uint32_t startQuery, uint32_t queryCount);
API_INTERFACE void SG_CALLCONV cmd_begin_query(Cmd* pCmd, QueryPool* pQueryPool, QueryDesc* pQuery);
API_INTERFACE void SG_CALLCONV cmd_end_query(Cmd* pCmd, QueryPool* pQueryPool, QueryDesc* pQuery);
API_INTERFACE void SG_CALLCONV cmd_resolve_query(Cmd* pCmd, QueryPool* pQueryPool, Buffer* pReadbackBuffer, uint32_t startQuery, uint32_t queryCount);

/************************************************************************/
// Stats Info Interface
/************************************************************************/
API_INTERFACE void SG_CALLCONV calculate_memory_stats(Renderer* pRenderer, char** stats);
API_INTERFACE void SG_CALLCONV calculate_memory_uUse(Renderer* pRenderer, uint64_t* usedBytes, uint64_t* totalAllocatedBytes);
API_INTERFACE void SG_CALLCONV free_memory_stats(Renderer* pRenderer, char* stats);

/************************************************************************/
// Debug Marker Interface
/************************************************************************/
API_INTERFACE void SG_CALLCONV cmd_begin_debug_marker(Cmd* pCmd, float r, float g, float b, const char* pName);
API_INTERFACE void SG_CALLCONV cmd_end_debug_marker(Cmd* pCmd);
API_INTERFACE void SG_CALLCONV cmd_add_debug_marker(Cmd* pCmd, float r, float g, float b, const char* pName);
#if defined(SG_GRAPHIC_API_D3D12)
API_INTERFACE uint32_t SG_CALLCONV cmd_write_marker(Cmd* pCmd, MarkerType markerType, uint32_t markerValue, Buffer* pBuffer, size_t offset, bool useAutoFlags = false);
#endif

/************************************************************************/
// Resource Debug Naming Interface
/************************************************************************/
API_INTERFACE void SG_CALLCONV set_buffer_name(Renderer* pRenderer, Buffer* pBuffer, const char* pName);
API_INTERFACE void SG_CALLCONV set_texture_name(Renderer* pRenderer, Texture* pTexture, const char* pName);
API_INTERFACE void SG_CALLCONV set_render_target_name(Renderer* pRenderer, RenderTarget* pRenderTarget, const char* pName);
API_INTERFACE void SG_CALLCONV set_pipeline_name(Renderer* pRenderer, Pipeline* pPipeline, const char* pName);

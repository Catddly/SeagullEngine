...
namespace SG
{
	...
	//
	SG_RENDER_API void SG_CALLCONV init_renderer(const char* appName, const RendererDesc* pSettings, Renderer** pRenderer);
	...
	SG_RENDER_API void SG_CALLCONV add_queue(Renderer* pRenderer, QueueDesc* pQueueDesc, Queue** pQueue);
	SG_RENDER_API void SG_CALLCONV remove_queue(Renderer* pRenderer, Queue* pQueue);

	SG_RENDER_API void SG_CALLCONV add_swap_chain(Renderer* pRenderer, const SwapChainDesc* pDesc, SwapChain** pSwapChain);
	SG_RENDER_API void SG_CALLCONV remove_swap_chain(Renderer* pRenderer, SwapChain* pSwapChain);

	// command pool functions
	SG_RENDER_API void SG_CALLCONV add_command_pool(Renderer* pRenderer, const CmdPoolDesc* pDesc, CmdPool** pCmdPool);
	...
	SG_RENDER_API void SG_CALLCONV add_render_target(Renderer* pRenderer, const RenderTargetDesc* pDesc, RenderTarget** ppRenderTarget);
	SG_RENDER_API void SG_CALLCONV remove_render_target(Renderer* pRenderer, RenderTarget* pRenderTarget);
	SG_RENDER_API void SG_CALLCONV add_sampler(Renderer* pRenderer, const SamplerDesc* pDesc, Sampler** pSampler);
	SG_RENDER_API void SG_CALLCONV remove_sampler(Renderer* pRenderer, Sampler* pSampler);

	// shader functions
	#if defined(TARGET_IOS)
	SG_RENDER_API void SG_CALLCONV add_shader(Renderer* pRenderer, const ShaderDesc* pDesc, Shader** p_shader_program);
	#endif
	SG_RENDER_API void SG_CALLCONV add_shader_binary(Renderer* pRenderer, const BinaryShaderDesc* pDesc, Shader** pShader);
	SG_RENDER_API void SG_CALLCONV remove_shader(Renderer* pRenderer, Shader* pShader);

	SG_RENDER_API void SG_CALLCONV add_root_signature(Renderer* pRenderer, const RootSignatureDesc* pDesc, RootSignature** pRootSignature);
	SG_RENDER_API void SG_CALLCONV remove_root_signature(Renderer* pRenderer, RootSignature* pRootSignature);

	// pipeline functions
	SG_RENDER_API void SG_CALLCONV add_pipeline(Renderer* pRenderer, const PipelineDesc* pPipelineSettings, Pipeline** pPipeline);
	...
	SG_RENDER_API void SG_CALLCONV add_descriptor_set(Renderer* pRenderer, const DescriptorSetDesc* pDesc, DescriptorSet** pDescriptorSet);
	SG_RENDER_API void SG_CALLCONV remove_descriptor_set(Renderer* pRenderer, DescriptorSet* pDescriptorSet);
	SG_RENDER_API void SG_CALLCONV update_descriptor_set(Renderer* pRenderer, uint32_t index, DescriptorSet* pDescriptorSet, uint32_t count, const DescriptorData* pParams);
	...
	SG_RENDER_API void SG_CALLCONV add_query_pool(Renderer* pRenderer, const QueryPoolDesc* pDesc, QueryPool** ppQueryPool);
	...
	SG_RENDER_API void SG_CALLCONV calculate_memory_uUse(Renderer* pRenderer, uint64_t* usedBytes, uint64_t* totalAllocatedBytes);

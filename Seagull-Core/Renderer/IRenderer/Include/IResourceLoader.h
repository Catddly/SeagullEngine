...
namespace SG
{
	...
	// MARK: add_resource and update_resource
	/// Adding and updating resources can be done using a add_resource or
	/// begin_update_resource/end_update_resource pair.
	/// if add_resource(BufferLoadDesc) is called with a data size larger than the ResourceLoader's staging buffer, the ResourceLoader
	/// will perform multiple copies/flushes rather than failing the copy. ( slicing the buffer )

	/// If token is NULL, the resource will be available when all_resource_loads_completed() returns true.

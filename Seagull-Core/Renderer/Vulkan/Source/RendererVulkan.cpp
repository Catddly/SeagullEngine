...
#include <../../../Core/Third-party/Include/eastl/include/EASTL/functional.h>
#include <../../../Core/Third-party/Include/eastl/include/EASTL/sort.h>
#include <../../../Core/Third-party/Include/eastl/include/EASTL/string_hash_map.h>
...
#pragma region (predefined macros)
// clang-format off
...
VkCompareOp gVkComparisonFuncTranslator[CompareMode::SG_SG_MAX_COMPARE_MODES] =
{
	VK_COMPARE_OP_NEVER,
	VK_COMPARE_OP_LESS,
	VK_COMPARE_OP_EQUAL,
...
VkStencilOp gVkStencilOpTranslator[StencilOp::SG_SG_MAX_STENCIL_OPS] =
{
	VK_STENCIL_OP_KEEP,
	VK_STENCIL_OP_ZERO,
...
{
	return sg_realloc(pOriginal, size);
}

static void VKAPI_PTR gVkFree(void* pUserData, void* pMemory) { sg_free(pMemory); }
...
#include <../include/Vulkan/vulkan-core.h>
...
static void remove_virtual_texture(Renderer* pRenderer, VirtualTexture* pTexture);
	...
	uint32_t numPoolSizes, DescriptorPool** ppPool)
{
	...
	poolCreateInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_createInfo;
	...
	CHECK_VKRESULT(vkCreateDescriptorPool(pPool->pVkDevice, &poolCreateInfo, &gVkAllocationCallbacks,
		pPool->pCurrentPool));
...
static void allocate_descriptor_sets(DescriptorPool* pPool, const VkDescriptorSetLayout* pLayouts, VkDescriptorSet** pSets, uint32_t nudescriptorsets)
{
	...
	DECLARE_ZERO(VkDescriptorSetAllocateInfo, allocInfo);
	...
	VkResult vkRes = vkAllocateDescriptorSets(pPool->pDevice, &allocInfo, *pSets);
	if (VK_SUCCESS != vkRes)
	{
		// try to recreate the pool
		VkDescriptorPool pDescriptorPool = VK_NULL_HANDLE;

		VkDescriptorPoolCreateInfo poolCreateInfo = { VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_createInfo ,nullptr };
		...
		allocInfo.descriptorPool = pPool->pCurrentPool;
		vkRes = vkAllocateDescriptorSets(pPool->pDevice, &allocInfo, *pSets);
	}
...
static void add_render_pass(Renderer* pRenderer, const RenderPassDesc* pDesc, RenderPass** ppRenderPass)
{
	...
	VkAttachmentReference* depthStencilAttachmentRef = NULL;
	...
	CHECK_VKRESULT(vkCreateRenderPass(pRenderer->pVkDevice, &createInfo, &gVkAllocationCallbacks, &(pRenderPass->pRenderPass)));
...
static void add_framebuffer(Renderer* pRenderer, const FrameBufferDesc* pDesc, FrameBuffer** ppFrameBuffer)
{
	...
	{
		pFrameBuffer->mArraySize = pDesc->ppRenderTargets[0]->depth;
	}
	...
	if (pDesc->pDepthStencil)
	{
		...
		else
		{
			...
			{
				if (pDesc->depthArraySlice != -1)
					handle = pDesc->depthMipSlice * pDesc->pDepthStencil->mArraySize + pDesc->depthArraySlice;
				else
	...
	DECLARE_ZERO(VkFramebufferCreateInfo, addInfo);
	addInfo.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_createInfo;
	addInfo.pNext = NULL;
	addInfo.flags = 0;
	addInfo.renderPass = pDesc->pRenderPass->pRenderPass;
	addInfo.attachmentCount = attachmentCount;
	addInfo.pAttachments = pImageViews;
	addInfo.width = pFrameBuffer->width;
	addInfo.height = pFrameBuffer->height;
	addInfo.layers = pFrameBuffer->mArraySize;
	CHECK_VKRESULT(vkCreateFramebuffer(pRenderer->pVkDevice, &addInfo, &gVkAllocationCallbacks, &(pFrameBuffer->pFramebuffer)));
...
#pragma region (Internal Log Modification)
...
#pragma endregion (Internal Log Modification)
...
#pragma region (Internal Utility Function)
...
#pragma endregion (Internal Utility Function)

#pragma region (Internal Init Functions)

void create_instance(const char* apname, const RendererDesc* pDesc, uint32_t userDefinedInstanceLayerCount, const char** userDefinedInstanceLayers,
	Renderer* pRenderer)
{
	...
	// Instance
	{
		...
		for (size_t i = 0; i < layerTemp.size(); ++i)
		{
			...
			VkExtensionProperties* properties = count ? (VkExtensionProperties*)sg_calloc(count, sizeof(*properties)) : NULL;
		...
		createInfo.sType = VK_STRUCTURE_TYPE_INSTANCE_createInfo;
#if VK_HEADER_VERSION >= 108
		...
		CHECK_VKRESULT(vkCreateInstance(&createInfo, &gVkAllocationCallbacks, &(pRenderer->pVkInstance)));
	}
...
//#endif
	...
	// Debug
	{
		...
		if (gDebugUtilsExtension)
		{
			...
			createInfo.sType = VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_createInfo_EXT;
			createInfo.pfnUserCallback = internal_debug_report_callback;
...
#endif
		{
			DECLARE_ZERO(VkDebugReportCallbackCreateInfoEXT, createInfo);
			createInfo.sType = VK_STRUCTURE_TYPE_DEBUG_REPORT_CALLBACK_createInfo_EXT;
			createInfo.pNext = NULL;
...
static bool add_device(const RendererDesc* pDesc, Renderer* pRenderer)
{
...
#if VK_KHR_device_group_creation
	VkDeviceGroupDeviceCreateInfoKHR deviceGroupInfo = { VK_STRUCTURE_TYPE_DEVICE_GROUP_DEVICE_createInfo_KHR };
	...
	if (pRenderer->gpuMode == SG_GPU_MODE_LINKED && gDeviceGroupCreationExtension)
	{
		...
		vkRes = vkEnumeratePhysicalDeviceGroupsKHR(pRenderer->pVkInstance, &deviceGroupCount, props);
		ASSERT(VK_SUCCESS == vkRes);
	...
	vkRes = vkEnumeratePhysicalDevices(pRenderer->pVkInstance, &gpuCount, NULL);
	ASSERT(VK_SUCCESS == vkRes);
	ASSERT(gpuCount);

	auto* gpus = (VkPhysicalDevice*)alloca(gpuCount * sizeof(VkPhysicalDevice));
	auto* gpuProperties = (VkPhysicalDeviceProperties2*)alloca(gpuCount * sizeof(VkPhysicalDeviceProperties2));
	auto* gpuMemoryProperties = (VkPhysicalDeviceMemoryProperties*)alloca(gpuCount * sizeof(VkPhysicalDeviceMemoryProperties));
	auto* gpuFeatures = (VkPhysicalDeviceFeatures2KHR*)alloca(gpuCount * sizeof(VkPhysicalDeviceFeatures2KHR));
	auto** queueFamilyProperties = (VkQueueFamilyProperties**)alloca(gpuCount * sizeof(VkQueueFamilyProperties*));
	auto* queueFamilyPropertyCount = (uint32_t*)alloca(gpuCount * sizeof(uint32_t));
	...
	auto* gpuSettings = (GPUSettings*)alloca(gpuCount * sizeof(GPUSettings));
	...
	*pRenderer->pActiveGpuSettings = gpuSettings[gpuIndex];
	...
	// Standalone extensions
	{
		...
		if (count > 0)
		{
			auto* properties = (VkExtensionProperties*)sg_calloc(count, sizeof(*properties));
			ASSERT(properties != NULL);
	...
	uint32_t queue_createInfos_count = 0;
	VkDeviceQueueCreateInfo queue_createInfos[4] = {};
	...
	for (uint32_t i = 0; i < queueFamiliesCount; i++)
	{
		...
		if (queueCount > 0)
		{
			...
			queue_createInfos[queue_createInfos_count].sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_createInfo;
			queue_createInfos[queue_createInfos_count].pNext = NULL;
			queue_createInfos[queue_createInfos_count].flags = 0;
			queue_createInfos[queue_createInfos_count].queueFamilyIndex = i;
			queue_createInfos[queue_createInfos_count].queueCount = queueCount;
			queue_createInfos[queue_createInfos_count].pQueuePriorities = queueFamilyPriorities[i];
			queue_createInfos_count++;
	...
	createInfo.sType = VK_STRUCTURE_TYPE_DEVICE_createInfo;
	createInfo.pNext = &gpuFeatures2;
	createInfo.flags = 0;
	createInfo.queueCreateInfoCount = queue_createInfos_count;
	createInfo.pQueueCreateInfos = queue_createInfos;
...
#endif
	CHECK_VKRESULT(vkCreateDevice(pRenderer->pVkActiveGPU, &createInfo, &gVkAllocationCallbacks, &pRenderer->pVkDevice));
...
#pragma endregion (Internal Init Functions)

#pragma region (Renderer)

void init_renderer(const char* apname, const RendererDesc* pDesc, Renderer** ppRenderer)
{
	ASSERT(apname);
	ASSERT(pDesc);
	ASSERT(ppRenderer);

	Renderer* pRenderer = (Renderer*)sg_calloc_memalign(1, alignof(Renderer), sizeof(Renderer));
	ASSERT(pRenderer);
...
//#endif
	...
	util_find_queue_family_index(pRenderer, 0, SG_QUEUE_TYPE_GRAPHICS, NULL, &pRenderer->graphicsQueueFamilyIndex, NULL);
	util_find_queue_family_index(pRenderer, 0, SG_QUEUE_TYPE_COMPUTE, NULL,  &pRenderer->computeQueueFamilyIndex, NULL);
	util_find_queue_family_index(pRenderer, 0, SG_QUEUE_TYPE_TRANSFER, NULL, &pRenderer->transferQueueFamilyIndex, NULL);
...
void add_fence(Renderer* pRenderer, Fence** ppFence)
{
	...
	Fence* pFence = (Fence*)sg_calloc(1, sizeof(Fence));
	ASSERT(pFence);

	DECLARE_ZERO(VkFenceCreateInfo, createInfo);
	createInfo.sType = VK_STRUCTURE_TYPE_FENCE_createInfo;
	createInfo.pNext = NULL;
	createInfo.flags = 0;
	CHECK_VKRESULT(vkCreateFence(pRenderer->pVkDevice, &createInfo, &gVkAllocationCallbacks, &pFence->pVkFence));
...
void add_semaphore(Renderer* pRenderer, Semaphore** ppSemaphore)
{
	...
	DECLARE_ZERO(VkSemaphoreCreateInfo, createInfo);
	createInfo.sType = VK_STRUCTURE_TYPE_SEMAPHORE_createInfo;
	createInfo.pNext = NULL;
	createInfo.flags = 0;
	CHECK_VKRESULT(vkCreateSemaphore(pRenderer->pVkDevice, &createInfo, &gVkAllocationCallbacks, &(pSemaphore->pVkSemaphore)));
...
void add_queue(Renderer* pRenderer, QueueDesc* pDesc, Queue** ppQueue)
{
	ASSERT(pDesc != NULL);
	...
	Queue* pQueue = (Queue*)sg_calloc(1, sizeof(Queue));
	ASSERT(pQueue);
...
void add_command_pool(Renderer* pRenderer, const CmdPoolDesc* pDesc, CmdPool** ppCmdPool)
{
	ASSERT(pRenderer);
	ASSERT(VK_NULL_HANDLE != pRenderer->pVkDevice);
	ASSERT(ppCmdPool);
	...
	DECLARE_ZERO(VkCommandPoolCreateInfo, createInfo);
	createInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_createInfo;
	createInfo.pNext = NULL;
	createInfo.flags = 0;
	...
	CHECK_VKRESULT(vkCreateCommandPool(pRenderer->pVkDevice, &createInfo, &gVkAllocationCallbacks, &(pCmdPool->pVkCmdPool)));
...
void add_cmd(Renderer* pRenderer, const CmdDesc* pDesc, Cmd** ppCmd)
{
	...
	Cmd* pCmd = (Cmd*)sg_calloc_memalign(1, alignof(Cmd), sizeof(Cmd));
	ASSERT(pCmd);
	...
	DECLARE_ZERO(VkCommandBufferAllocateInfo, allocInfo);
	...
	CHECK_VKRESULT(vkAllocateCommandBuffers(pRenderer->pVkDevice, &allocInfo, &(pCmd->pVkCmdBuf)));
...
void add_swap_chain(Renderer* pRenderer, const SwapChainDesc* pDesc, SwapChain** ppSwapChain)
{
	ASSERT(pRenderer);
	ASSERT(pDesc);
	ASSERT(ppSwapChain);
	ASSERT(pDesc->mImageCount <= SG_MAX_SWAPCHAIN_IMAGES);
...
#if defined(VK_USE_PLATFORM_WIN32_KHR)
	DECLARE_ZERO(VkWin32SurfaceCreateInfoKHR, createInfo);
	createInfo.sType = VK_STRUCTURE_TYPE_WIN32_SURFACE_createInfo_KHR;
	createInfo.pNext = NULL;
	createInfo.flags = 0;
	createInfo.hinstance = ::GetModuleHandle(NULL);
	createInfo.hwnd = (HWND)pDesc->windowHandle.window;
	CHECK_VKRESULT(vkCreateWin32SurfaceKHR(pRenderer->pVkInstance, &createInfo, &gVkAllocationCallbacks, &vkSurface)); // surface to present
#elif defined(VK_USE_PLATFORM_XLIB_KHR)
	DECLARE_ZERO(VkXlibSurfaceCreateInfoKHR, createInfo);
	...
	CHECK_VKRESULT(vkCreateXlibSurfaceKHR(pRenderer->pVkInstance, &createInfo, &gVkAllocationCallbacks, &vkSurface));
#elif defined(VK_USE_PLATFORM_XCB_KHR)
	DECLARE_ZERO(VkXcbSurfaceCreateInfoKHR, createInfo);
	createInfo.sType = VK_STRUCTURE_TYPE_XCB_SURFACE_createInfo_KHR;
	createInfo.pNext = NULL;
	createInfo.flags = 0;
	createInfo.connection = pDesc->windowHandle.connection;    //TODO
	createInfo.window = pDesc->windowHandle.window;        //TODO
	CHECK_VKRESULT(vkCreateXcbSurfaceKHR(pRenderer->pVkInstance, &createInfo, &gVkAllocationCallbacks, &vkSurface));
#elif defined(VK_USE_PLATFORM_IOS_MVK)
	// Add IOS support here
#elif defined(VK_USE_PLATFORM_MACOS_MVK)
	// Add MacOS support here
#elif defined(VK_USE_PLATFORM_ANDROID_KHR)
	DECLARE_ZERO(VkAndroidSurfaceCreateInfoKHR, createInfo);
	createInfo.sType = VK_STRUCTURE_TYPE_ANDROID_SURFACE_createInfo_KHR;
	createInfo.pNext = NULL;
	createInfo.flags = 0;
	createInfo.window = pDesc->windowHandle.window;
	CHECK_VKRESULT(vkCreateAndroidSurfaceKHR(pRenderer->pVkInstance, &createInfo, &gVkAllocationCallbacks, &vkSurface));
#elif defined(VK_USE_PLATFORM_GGP)
...
#error PLATFORM NOT SUPPORTED
	...
	{
		SG_LOG_WARING("Changed requested SwapChain images {%u} to maximum allowed SwapChain images {%u}", pDesc->mImageCount, caps.maxImageCount);
		((SwapChainDesc*)pDesc)->mImageCount = caps.maxImageCount;
	}
	if (pDesc->mImageCount < caps.minImageCount)
	{
		SG_LOG_WARING("Changed requested SwapChain images {%u} to minimum required SwapChain images {%u}", pDesc->mImageCount, caps.minImageCount);
	...
	// Swapchain
	VkExtent2D extent = { 0 };
	extent.width = clamp(pDesc->width, caps.minImageExtent.width, caps.maxImageExtent.width);
	extent.height = clamp(pDesc->height, caps.minImageExtent.height, caps.maxImageExtent.height);
	...
	swapChainCreateInfo.sType = VK_STRUCTURE_TYPE_SWAPCHAIN_createInfo_KHR;
	...
	auto* pSwapChain = (SwapChain*)sg_calloc(1, sizeof(SwapChain) + imageCount * sizeof(RenderTarget*) + sizeof(SwapChainDesc));
	...
	for (uint32_t i = 0; i < imageCount; ++i)
	{
		...
		barriers[i] = { pSwapChain->ppRenderTargets[i], SG_RESOURCE_STATE_UNDEFINED, SG_RESOURCE_STATE_PRESENT };
	}
	...
	begin_cmd(cmd);
	cmd_resource_barrier(cmd, 0, NULL, 0, NULL, imageCount, barriers);
	end_cmd(cmd);
...
void remove_swap_chain(Renderer* pRenderer, SwapChain* pSwapChain)
{
	ASSERT(pRenderer);
	ASSERT(pSwapChain);
...
void add_buffer(Renderer* pRenderer, const BufferCreateDesc* pDesc, Buffer** ppBuffer)
{
	...
	DECLARE_ZERO(VkBufferCreateInfo, addInfo);
	addInfo.sType = VK_STRUCTURE_TYPE_BUFFER_createInfo;
	addInfo.pNext = NULL;
	addInfo.flags = 0;
	addInfo.size = allocationSize;
	addInfo.usage = util_to_vk_buffer_usage(pDesc->descriptors, pDesc->mFormat != TinyImageFormat_UNDEFINED);
	...
	const bool linkedMultiGpu = (pRenderer->gpuMode == SG_GPU_MODE_LINKED && (pDesc->pSharedNodeIndices || pDesc->mNodeIndex));

	VmaAllocationCreateInfo vmaMemReqs = { 0 };
	vmaMemReqs.usage = (VmaMemoryUsage)pDesc->memoryUsage;
	vmaMemReqs.flags = 0;
	if (pDesc->mFlags & SG_BUFFER_CREATION_FLAG_OWN_MEMORY_BIT)
		vmaMemReqs.flags |= VMA_ALLOCATION_CREATE_DEDICATED_MEMORY_BIT;
	if (pDesc->mFlags & SG_BUFFER_CREATION_FLAG_PERSISTENT_MAP_BIT)
		vmaMemReqs.flags |= VMA_ALLOCATION_CREATE_MAPPED_BIT;
	if (linkedMultiGpu)
		vmaMemReqs.flags |= VMA_ALLOCATION_CREATE_DONT_BIND_BIT;

	VmaAllocationInfo allocInfo = {};
	CHECK_VKRESULT(vmaCreateBuffer(pRenderer->pVmaAllocator, &addInfo, &vmaMemReqs,
		&pBuffer->pVkBuffer, &pBuffer->pVkAllocation, &allocInfo));
	...
	if (linkedMultiGpu)
	{
		...
		util_calculate_device_indices(pRenderer, pDesc->mNodeIndex, pDesc->pSharedNodeIndices, pDesc->sharedNodeIndexCount, pIndices);
		/************************************************************************/
		...
		(pDesc->descriptors & SG_DESCRIPTOR_TYPE_RW_BUFFER))
	{
		...
		{
			pBuffer->mOffset = pDesc->structStride * pDesc->firstElement;
		}
	}

	if (addInfo.usage & VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT)
	{
		VkBufferViewCreateInfo viewInfo = { VK_STRUCTURE_TYPE_BUFFER_VIEW_createInfo, nullptr };
		...
		{
			SG_LOG_WARING("Failed to create uniform texel buffer view for format %u", (uint32_t)pDesc->mFormat);
		}
		else
		{
	...
	{
		VkBufferViewCreateInfo viewInfo = { VK_STRUCTURE_TYPE_BUFFER_VIEW_createInfo, NULL };
		viewInfo.buffer = pBuffer->pVkBuffer;
		viewInfo.flags = 0;
		viewInfo.format = (VkFormat)TinyImageFormat_ToVkFormat(pDesc->mFormat);
		viewInfo.offset = pDesc->firstElement * pDesc->structStride;
		...
		{
			SG_LOG_WARING("Failed to create storage texel buffer view for format %u", (uint32_t)pDesc->mFormat);
		}
		else
		{
...
{
	ASSERT(pRenderer);
	ASSERT(pDesc && pDesc->width && pDesc->height && (pDesc->depth || pDesc->mArraySize));
	...
	Texture* pTexture = (Texture*)sg_calloc_memalign(1, alignof(Texture), totalSize);
	ASSERT(pTexture);

	if (pDesc->descriptors & SG_DESCRIPTOR_TYPE_RW_TEXTURE)
		pTexture->pVkUAVDescriptors = (VkImageView*)(pTexture + 1);

	if (pDesc->pNativeHandle && !(pDesc->mFlags & SG_TEXTURE_CREATION_FLAG_IMPORT_BIT))
	{
	...
	if (pDesc->mFlags & SG_TEXTURE_CREATION_FLAG_FORCE_2D)
	{
		ASSERT(pDesc->depth == 1);
		imageType = VK_IMAGE_TYPE_2D;
	}
	else if (pDesc->mFlags & SG_TEXTURE_CREATION_FLAG_FORCE_3D)
	{
	...
	{
		DECLARE_ZERO(VkImageCreateInfo, createInfo);
		createInfo.sType = VK_STRUCTURE_TYPE_IMAGE_createInfo;
		createInfo.pNext = NULL;
		createInfo.flags = 0;
		createInfo.imageType = imageType;
		createInfo.format = (VkFormat)TinyImageFormat_ToVkFormat(pDesc->mFormat);
		...
		createInfo.arrayLayers = pDesc->mArraySize;
		...
		if (pDesc->mFlags & SG_TEXTURE_CREATION_FLAG_OWN_MEMORY_BIT)
		...
		VkExternalMemoryImageCreateInfoKHR externalInfo = { VK_STRUCTURE_TYPE_EXTERNAL_MEMORY_IMAGE_createInfo_KHR, NULL };
#if defined(VK_USE_PLATFORM_WIN32_KHR)
		...
		if (gExternalMemoryExtension && pDesc->mFlags & SG_TEXTURE_CREATION_FLAG_IMPORT_BIT)
		{
		...
		else if (gExternalMemoryExtension && pDesc->mFlags & SG_TEXTURE_CREATION_FLAG_EXPORT_BIT)
		{
#if defined(VK_USE_PLATFORM_WIN32_KHR)
		...
		if (isSinglePlane)
		{
			CHECK_VKRESULT(vmaCreateImage(pRenderer->pVmaAllocator, &createInfo, &memReqs,
		...
		else // Multi-planar formats
		{
			...
			formatList.sType = VK_STRUCTURE_TYPE_IMAGE_FORMAT_LIST_createInfo_KHR;
			...
			// Create Image
			CHECK_VKRESULT(vkCreateImage(pRenderer->pVkDevice, &createInfo, &gVkAllocationCallbacks, &pTexture->pVkImage));
			...
			CHECK_VKRESULT(vkAllocateMemory(pRenderer->pVkDevice, &mem_allocInfo, &gVkAllocationCallbacks, &pTexture->pVkDeviceMemory));
	...
	switch (imageType)
	{
		case VK_IMAGE_TYPE_1D: viewType = pDesc->mArraySize > 1 ? VK_IMAGE_VIEW_TYPE_1D_ARRAY : VK_IMAGE_VIEW_TYPE_1D; break;
		case VK_IMAGE_TYPE_2D:
			if (cubemapRequired)
				viewType = (pDesc->mArraySize > 6) ? VK_IMAGE_VIEW_TYPE_CUBE_ARRAY : VK_IMAGE_VIEW_TYPE_CUBE;
			else
	...
	// SRV
	srvDesc.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_createInfo;
	srvDesc.pNext = NULL;
	srvDesc.flags = 0;
	...
	pTexture->arraySizeMinusOne = pDesc->mArraySize - 1;
...
void add_render_target(Renderer* pRenderer, const RenderTargetDesc* pDesc, RenderTarget** ppRenderTarget)
{
	ASSERT(pRenderer);
	ASSERT(pDesc);
	...
	uint32_t depthOrArraySize = pDesc->mArraySize * pDesc->depth;
	...
	if (isDepth)
	{
		...
		if (VK_FORMAT_UNDEFINED != vk_depth_stencil_format)
		{
			...
			if (VK_SUCCESS != vkRes)
			{
				...
				SG_LOG_WARING("Depth stencil format (%u) not supported. Falling back to D16 format", pDesc->mFormat);
			}
		}
	}
	...
	VkImageViewCreateInfo rtvDesc = { VK_STRUCTURE_TYPE_IMAGE_VIEW_createInfo, NULL };
	rtvDesc.flags = 0;
	...
	pRenderTarget->arraySize = pDesc->mArraySize;
...
void remove_render_target(Renderer* pRenderer, RenderTarget* pRenderTarget)
{
	...
	const uint32_t depthOrArraySize = pRenderTarget->mArraySize * pRenderTarget->depth;
...
void add_sampler(Renderer* pRenderer, const SamplerDesc* pDesc, Sampler** ppSampler)
{
	ASSERT(pRenderer);
	ASSERT(VK_NULL_HANDLE != pRenderer->pVkDevice);
	ASSERT(pDesc->mCompareFunc < SG_MAX_COMPARE_MODES);
	ASSERT(ppSampler);

	auto* pSampler = (Sampler*)sg_calloc_memalign(1, alignof(Sampler), sizeof(Sampler));
	ASSERT(pSampler);

	DECLARE_ZERO(VkSamplerCreateInfo, createInfo);
	...
	createInfo.mipmapMode = util_to_vk_mip_map_mode(pDesc->mMipMapMode);
	...
	createInfo.compareEnable = (gVkComparisonFuncTranslator[pDesc->mCompareFunc] != VK_COMPARE_OP_NEVER) ? VK_TRUE : VK_FALSE;
	createInfo.compareOp = gVkComparisonFuncTranslator[pDesc->mCompareFunc];

	createInfo.minLod = 0.0f;
	createInfo.maxLod = ((pDesc->mMipMapMode == SG_MIPMAP_MODE_LINEAR) ? FLT_MAX : 0.0f);
	...
	if (TinyImageFormat_IsPlanar(pDesc->mSamplerConversionDesc.mFormat))
	{
		...
		DECLARE_ZERO(VkSamplerYcbcrConversionCreateInfo, conversionInfo);
		conversionInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_YCBCR_CONVERSION_createInfo;
		...
		CHECK_VKRESULT(vkCreateSamplerYcbcrConversion(pRenderer->pVkDevice, &conversionInfo, &gVkAllocationCallbacks, &pSampler->pVkSamplerYcbcrConversion));
	...
	CHECK_VKRESULT(vkCreateSampler(pRenderer->pVkDevice, &createInfo, &gVkAllocationCallbacks, &(pSampler->pVkSampler)));
...
void add_descriptor_set(Renderer* pRenderer, const DescriptorSetDesc* pDesc, DescriptorSet** pDescriptorSet)
{
	ASSERT(pRenderer);
	ASSERT(pDesc);
	...
	if (VK_NULL_HANDLE != pRootSignature->vkDescriptorSetLayouts[updateFreq])
	{
		...
		allocate_descriptor_sets(pRenderer->pDescriptorPool, pLayouts, pHandles, pDesc->mMaxSets);
	}
	else
	{
...
#define VALIDATE_DESCRIPTOR(descriptor, ...)
	...
	for (uint32_t i = 0; i < count; ++i)
	{
		...
		VALIDATE_DESCRIPTOR(pDesc->mUpdateFrequency == updateFreq, "Descriptor (%s) - Mismatching update frequency and set index", pDesc->name);
		...
		case SG_DESCRIPTOR_TYPE_SAMPLER:
		{
			...
			VALIDATE_DESCRIPTOR(pDesc->indexInParent != -1,
				"Trying to update a static sampler (%s). All static samplers must be set in add_root_signature and cannot be updated later",
				pDesc->name);
		...
		case SG_DESCRIPTOR_TYPE_UNIFORM_BUFFER:
		{
			if (pDesc->vkType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC)
			{
				...
				VALIDATE_DESCRIPTOR(pParam->sizes[0] <= pRenderer->pVkActiveGPUProperties->properties.limits.maxUniformBufferRange,
					...
					pParam->sizes[0], pRenderer->pVkActiveGPUProperties->properties.limits.maxUniformBufferRange);
				...
				{
					pParam->ppBuffers[0]->pVkBuffer,  // buffer
					pParam->ppBuffers[0]->offset,    // offset
					pParam->sizes[0]                 // range
				};
		...
		case SG_DESCRIPTOR_TYPE_RW_BUFFER_RAW:
		{
			...
			for (uint32_t arr = 0; arr < arrayCount; ++arr)
			{
				...
				if (pParam->offsets)
				{
					VALIDATE_DESCRIPTOR(pParam->sizes, "Descriptor (%s) - pSizes must be provided with offsets", pDesc->name);
		...
		case SG_DESCRIPTOR_TYPE_RAY_TRACING:
		{
			...
			for (uint32_t arr = 0; arr < arrayCount; ++arr)
			{
				...
				vk_fill_raytracing_descriptor_data(pParam->ppAccelerationStructures[arr], pWriteNV);
...
#pragma region (Bind Shader Resources)
...
void cmd_bind_descriptor_set(Cmd* pCmd, uint32_t index, DescriptorSet* pDescriptorSet)
{
	...
	vkCmdBindDescriptorSets(pCmd->pVkCmdBuf, gPipelineBindPoint[pRootSignature->pipelineType], pRootSignature->pPipelineLayout, pDescriptorSet->mUpdateFrequency, 1, &pDescriptorSet->pHandles[index],
...
void cmd_bind_push_constants_by_index(Cmd* pCmd, RootSignature* pRootSignature, uint32_t paramIndex, const void* pConstants)
{
	...
	ASSERT(pDesc);
	ASSERT(SG_DESCRIPTOR_TYPE_ROOT_CONSTANT == pDesc->mType);

	vkCmdPushConstants(pCmd->pVkCmdBuf, pRootSignature->pPipelineLayout,
		pDesc->vkStages, 0, pDesc->size, pConstants);
}

#pragma endregion (Bind Shader Resources)

#pragma region (Shader Function)

void add_shader_binary(Renderer* pRenderer, const BinaryShaderDesc* pDesc, Shader** ppShaderProgram)
{
	ASSERT(pRenderer);
	ASSERT(pDesc);
	...
	for (uint32_t i = 0; i < SG_SHADER_stageCount; ++i)
	{
	...
	counter = 0;
	ShaderReflection stageReflections[SG_SHADER_stageCount] = {};
	...
	for (uint32_t i = 0; i < SG_SHADER_stageCount; ++i)
	{
		...
		{
			DECLARE_ZERO(VkShaderModuleCreateInfo, createInfo);
			...
			case SG_SHADER_STAGE_VERT:
			{
				...
				CHECK_VKRESULT(vkCreateShaderModule(pRenderer->pVkDevice, &createInfo, &gVkAllocationCallbacks, &(pShaderProgram->pShaderModules[counter])));
			}
			break;
...
void add_root_signature(Renderer* pRenderer, const RootSignatureDesc* pRootSignatureDesc, RootSignature** ppRootSignature)
{
	ASSERT(pRenderer);
	...
	VkPushConstantRange                      pushConstants[SG_SHADER_stageCount] = {};
	...
	for (uint32_t i = 0; i < (uint32_t)shaderResources.size(); ++i)
	{
		...
		if (pDesc->type != SG_DESCRIPTOR_TYPE_ROOT_CONSTANT)
		{
			...
			if (name.find("rootcbv") != eastl::string::npos)
			{
				...
				{
					SG_LOG_WARING("Descriptor (%s) : Cannot use VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC for arrays",
						pDesc->name);
				}
			}
			...
			if (hasStaticSampler)
			{
				SG_LOG_INFO("Descriptor (%s) : User specified Static Sampler", pDesc->pName);
		...
		{
			SG_LOG_INFO("Descriptor (%s) : User specified Push Constant", pDesc->pName);
	...
	for (uint32_t i = kMaxLayoutCount; i-- > 0U;) // the number is bigger, the frequency is higher
	{
		...
		if (createLayout)
		{
			...
			CHECK_VKRESULT(vkCreateDescriptorSetLayout(pRenderer->pVkDevice, &layoutInfo, &gVkAllocationCallbacks, &pRootSignature->vkDescriptorSetLayouts[i]));
		}
	...
	DECLARE_ZERO(VkPipelineLayoutCreateInfo, addInfo);
	...
	CHECK_VKRESULT(vkCreatePipelineLayout(pRenderer->pVkDevice, &addInfo, &gVkAllocationCallbacks, &(pRootSignature->pPipelineLayout)));
	...
	for (uint32_t setIndex = 0; setIndex < SG_DESCRIPTOR_UPDATE_FREQ_COUNT; ++setIndex)
	{
		if (pRootSignature->vkDescriptorCounts[setIndex])
		{
			...
			VkDescriptorUpdateTemplateCreateInfoKHR createInfo = { VK_STRUCTURE_TYPE_DESCRIPTOR_UPDATE_TEMPLATE_CREATE_INFO_KHR, nullptr };
			...
			CHECK_VKRESULT(vkCreateDescriptorUpdateTemplateKHR(pRenderer->pVkDevice, &createInfo, &gVkAllocationCallbacks, &pRootSignature->updateTemplates[setIndex]));
...
void remove_root_signature(Renderer* pRenderer, RootSignature* pRootSignature)
{
	for (uint32_t i = 0; i < SG_DESCRIPTOR_UPDATE_FREQ_COUNT; ++i)
	{
		vkDestroyDescriptorSetLayout(pRenderer->pVkDevice, pRootSignature->vkDescriptorSetLayouts[i], &gVkAllocationCallbacks);
		if (VK_NULL_HANDLE != pRootSignature->updateTemplates[i])
			vkDestroyDescriptorUpdateTemplateKHR(pRenderer->pVkDevice, pRootSignature->updateTemplates[i], &gVkAllocationCallbacks);
...
static void add_graphic_pipeline(Renderer* pRenderer, const PipelineDesc* pMainDesc, Pipeline** ppPipeline)
{
	ASSERT(pRenderer);
	ASSERT(ppPipeline);
	ASSERT(pMainDesc);
	...
	// Pipeline
	{
		...
		if (pVertexLayout != nullptr)
		{
			...
			for (uint32_t i = 0; i < attribCount; ++i)
			{
				...
				inputBindings[inputBindingCount - 1].stride += TinyImageFormat_BitSizeOfBlock(attrib->mFormat) / 8;

				inputAttributes[inputAttributeCount].location = attrib->location;
				inputAttributes[inputAttributeCount].binding = attrib->binding;
				inputAttributes[inputAttributeCount].format = (VkFormat)TinyImageFormat_ToVkFormat(attrib->mFormat);
		...
		DECLARE_ZERO(VkPipelineColorBlendAttachmentState, cbAtt[SG_MAX_RENDER_TARGET_ATTACHMENTS]);
		...
		DECLARE_ZERO(VkDynamicState, dynamicStates[6]);
		...
		DECLARE_ZERO(VkGraphicsPipelineCreateInfo, createInfo);
		...
		if ((pShaderProgram->stages & SG_SHADER_STAGE_TESC) && (pShaderProgram->stages & SG_SHADER_STAGE_TESE))
			...
			createInfo.pTessellationState = NULL;    // set tessellation state to null if we have no tessellation
		...
		CHECK_VKRESULT(vkCreateGraphicsPipelines(pRenderer->pVkDevice, psoCache, 1, &createInfo, &gVkAllocationCallbacks, &(pPipeline->pVkPipeline)));
...
static void add_compute_pipeline(Renderer* pRenderer, const PipelineDesc* pMainDesc, Pipeline** ppPipeline)
{
	ASSERT(pRenderer);
	ASSERT(ppPipeline);
	ASSERT(pMainDesc);
	...
	ASSERT(pPipeline);
	pPipeline->mType = SG_PIPELINE_TYPE_COMPUTE;

	{
		...
		DECLARE_ZERO(VkComputePipelineCreateInfo, createInfo);
		...
		CHECK_VKRESULT(vkCreateComputePipelines(pRenderer->pVkDevice, psoCache, 1, &createInfo, &gVkAllocationCallbacks, &(pPipeline->pVkPipeline)));
	}

	*ppPipeline = pPipeline;
}

void add_pipeline(Renderer* pRenderer, const PipelineDesc* pDesc, Pipeline** ppPipeline)
{
	switch (pDesc->type)
	{
...
void begin_cmd(Cmd* pCmd)
{
	...
	DECLARE_ZERO(VkCommandBufferBeginInfo, beginInfo);
	...
	const LoadActionsDesc* pLoadActions, uint32_t* pColorArraySlices, uint32_t* pColorMipSlices, uint32_t depthArraySlice, uint32_t depthMipSlice)
{
	...
	DECLARE_ZERO(VkRect2D, renderArea);
	...
	DECLARE_ZERO(VkRenderPassBeginInfo, beginInfo);
...
void cmd_update_subresource(Cmd* pCmd, Texture* pTexture, Buffer* pSrcBuffer, const SubresourceDataDesc* pSubresourceDesc)
{
	...
	if (isSinglePlane)
	{
		const uint32_t width = max<uint32_t>(1, pTexture->width >> pSubresourceDesc->mipLevel);
		const uint32_t height = max<uint32_t>(1, pTexture->height >> pSubresourceDesc->mipLevel);
		const uint32_t depth = max<uint32_t>(1, pTexture->depth >> pSubresourceDesc->mipLevel);
...
#pragma endregion (Command Buffer Functions)

#pragma endregion (Renderer Implementation)

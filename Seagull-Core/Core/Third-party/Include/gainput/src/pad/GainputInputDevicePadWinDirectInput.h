...
#define guide "guide"
	...
	static void PrintControllerInfo(const DIDEVICEINSTANCE * pdidInstance)
	{
		...
		SG_LOG_DEBUG("GamePad Name: %s , GUID: %s, Product Id: %d, Vendor Id: %d", pdidInstance->tszProductName, guid_string, product, vendor);
	}
	...
	*/
	...
	public:
		...
		bool Init(void* _hwnd)
		{
			...
			if (FAILED(re))
			{
				SG_LOG_ERROR("CoCreateInstance failed for gamePad %08x", re);
				return false;
			}
			if (!gamePads.dinput)
			{
				SG_LOG_ERROR("gamePads.dinput is null");
				return false;
			}
			//initialize
	...
	public:
		...
		bool SetRumbleEffectFeedback(const ControllerFeedback& data)
		{
			...
			{
				SG_LOG_ERROR("gamepadInfo.hidDevice == INVALID_HANDLE_VALUE");
				return false;
			}
			...
			if (result != TRUE)
			{
				SG_LOG_ERROR("result != TRUE");
				return false;
			}
			if (bytes_written != ARRAYSIZE(buf))
			{
				SG_LOG_ERROR("bytes_written != ARRAYSIZE(buf)");
				return false;
			}
			return result;
		}

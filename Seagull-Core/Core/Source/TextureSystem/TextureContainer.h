...
namespace SG
{
		...
		uint32_t* outNumBytes, uint32_t* outRowBytes, uint32_t* outNumRows)
	{
		...
		if (compressed)
		{
			...
			if (width > 0)
			{
				numBlocksWide = eastl::max(1U, (width + (blockWidth - 1)) / blockWidth);
			}
			if (height > 0)
			{
				numBlocksHigh = eastl::max(1u, (height + (blockHeight - 1)) / blockHeight);
			}
...
#define ISBITMASK( r,g,b,a ) ( ddpf.RBitMask == r && ddpf.GBitMask == g && ddpf.BBitMask == b && ddpf.ABitMask == a )
	...
	static bool load_dds_texture(FileStream* pStream, TextureCreateDesc* pOutDesc)
	{
...
{                             \
	return false;             \
		...
		ssize_t ddsDataSize = sgfs_get_stream_file_size(pStream);
		...
		sgfs_read_from_stream(pStream, &dwMagicNumber, sizeof(dwMagicNumber));
		...
		DDS_HEADER_DXT10* d3d10ext = nullptr;

		ssize_t bytesRead = sgfs_read_from_stream(pStream, header, sizeof(DDS_HEADER));
			...
			(MAKEFOURCC('D', 'X', '1', '0') == header->ddspf.fourCC))
		{
			...
			bytesRead = sgfs_read_from_stream(pStream, d3d10ext, sizeof(DDS_HEADER_DXT10));
		...
		textureDesc.mipLevels = eastl::max(1U, header->mipMapCount);
		textureDesc.descriptors = SG_DESCRIPTOR_TYPE_TEXTURE;
		textureDesc.sampleCount = SG_SAMPLE_COUNT_1;

		if (d3d10ext)
		{
			textureDesc.arraySize = d3d10ext->arraySize;
			RETURN_IF_FAILED(textureDesc.arraySize);

			textureDesc.format = TinyImageFormat_FromDXGI_FORMAT(d3d10ext->dxgiFormat);
			...
			default:
				RETURN_IF_FAILED(TinyImageFormat_BitSizeOfBlock(textureDesc.format) != 0);
			}
			...
			case D3D12_RESOURCE_DIMENSION_TEXTURE1D:
				...
				if ((header->flags & DDS_HEIGHT) && textureDesc.height != 1)
				{
					RETURN_IF_FAILED(false);
				}
				textureDesc.height = textureDesc.depth = 1;
				break;
		...
		else
		{
			...
			textureDesc.format = TinyImageFormat_FromDXGI_FORMAT(dxgiFormat);
			RETURN_IF_FAILED(textureDesc.format != TinyImageFormat_UNDEFINED);
			RETURN_IF_FAILED(TinyImageFormat_BitSizeOfBlock(textureDesc.format) != 0);
...
{                             \
	return false;             \
		...
		ssize_t svtDataSize = sgfs_get_stream_file_size(pStream);
		RETURN_IF_FAILED(svtDataSize <= UINT32_MAX);
		RETURN_IF_FAILED((svtDataSize > (sizeof(SVT_HEADER))));

		SVT_HEADER header = {};
		ssize_t bytesRead = sgfs_read_from_stream(pStream, &header, sizeof(SVT_HEADER));
